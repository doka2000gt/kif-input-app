...
基本:
  ...
  p 72 vK 74 P 64 G     複数指定で一括配置（途中エラーなら全部キャンセル）
  p 55 .               駒を消す
  h b G 1              先手の持駒（例: 金1枚）
  h b G 1 N 1 L 1       複数指定で一括設定（途中エラーなら全部キャンセル）
...
形式:
  ...
  p 55 .         : 駒を消す
  p 72 vK 74 P 64 G     : 複数指定（スペース区切りで (マス 駒) を繰り返す）

注意:
  - 複数指定は「途中でエラーが出たら全部キャンセル（atomic）」です
...
形式:
  ...
  h b G 1 N 1 L 1       : 複数指定（スペース区切りで (駒 枚数) を繰り返す）
