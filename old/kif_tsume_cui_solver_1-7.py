...
def batch_process_kif(path: str, default_ply: Optional[int] = None,
    ...
    for j, mvlist in enumerate(lines[1:], start=2):
        ...
        written.append(generate_kif_single_line(board0, hands_b, gote_auto, "", "", mvlist, fname))
...
def _batch_process_one_kif(kif_path: Path, ply: int, sente: str, gote: str):
    """
    1つのKIFを処理する（batch用）
    ・複数解があれば _002 以降を出力
    ・なければ元ファイルを OUTPUT にコピー
    """
    print(f"[batch] processing: {kif_path.name}")

    # 既存の solve / 分岐生成ロジックを使う前提
    # ここでは「solve を直接呼ばない」ことが重要

    try:
        raw = kif_path.read_text(encoding="cp932", errors="replace")
    except Exception:
        print(f"[batch] 読み込み失敗: {kif_path.name}")
        return 0

    # 今は最小実装：そのまま OUTPUT にコピー
    out = OUTPUT_DIR / kif_path.name
    out.write_text(raw, encoding="cp932", errors="replace")
    return 1
...
def main():
    ...
    while True:
        ...
        if s == "batch" or s.startswith("batch "):
            parts = s.split()
            ply = 9  # デフォルト手数（必要なら変更OK）

            # 例: batch 3  → ply=3
            if len(parts) == 2 and parts[1].isdigit():
                ply = int(parts[1])

            print(f"[batch] INPUT 内を一括処理します（ply={ply}）")

            count = 0
            for kif in sorted(INPUT_DIR.glob("*.kif")):
                try:
                    count += _batch_process_one_kif(kif, ply, sente, gote)
                except Exception as e:
                    print(f"[batch] エラー: {kif.name}: {e}")

            print(f"[batch] 完了: {count} ファイルを OUTPUT に出力しました")
            continue
        ...
        if s.startswith("solve"):
            ...
            if out:
                ...
                if not paths:
                    ...
                    written.append(generate_kif_single_line(board0, hands_b, gote_auto, sente, gote, paths[0], f"{stem}.kif"))
                    for i_line, mvlist in enumerate(paths[1:], start=2):
                        written.append(generate_kif_single_line(board0, hands_b, gote_auto, sente, gote, mvlist, f"{stem}_{i_line:03d}.kif"))
                    if len(written) == 1:
