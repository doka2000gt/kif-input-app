...
def _piece_type_to_kind(pt: int) -> str:
    # Map python-shogi piece type to our kind letter
    # constants: PAWN,LANCE,KNIGHT,SILVER,GOLD,BISHOP,ROOK,KING
    mapping = {
    ...
    if pt not in mapping:
        # Fallback: try numeric known order (rare)
        return "P"
    return mapping[pt]
...
def kif_line_for_shogi_move(idx: int, board_before, move, prev_to: Optional[Tuple[int,int]], sec: int, total_sec: int) -> Tuple[str, Tuple[int,int]]:
    ...
    if move.drop_piece_type is not None:
        ...
        if move.promotion:
            ...
            if p.promoted and kind in PROMOTED_JP:
    ...
    for r in range(1,10):
        ...
        for f in range(9,0,-1):
            ...
            if p is None:
                ...
                kind = _piece_type_to_kind(p.piece_type)
                name = PROMOTED_JP[kind] if (p.promoted and kind in PROMOTED_JP) else PIECE_JP[kind]
