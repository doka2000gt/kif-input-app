...
HELP_END = """\
[help end]
終局理由（末尾行）を選びます。詰将棋用途なら通常は「詰み」です。

  end        : 対話式に選択（1:詰み / 2:投了）
  end mate   : 末尾行を「詰み」にする
  end resign : 末尾行を「投了」にする

※保存(s ...)したとき、最後の行がこの結果になります。
"""
...
# ---- remaining pieces helper (for tsume KIF style) ----
TOTAL_POOL = {"R":2, "B":2, "G":4, "S":4, "N":4, "L":4, "P":18}

def remaining_gote_hand_from_setup(board: Dict[Tuple[int,int], Optional["Piece"]],
                                   sente_hand: Dict[str,int]) -> Dict[str,int]:
    """Compute '後手の持駒' as the remaining pool pieces not present on board nor in sente hand.
    This matches common tsume-shogi KIF style (see 015.kif).
    Kings are excluded from hands.
    Promoted pieces on board count as their base kind.
    """
    used = {k:0 for k in TOTAL_POOL}
    for sq, p in board.items():
        ...
        if k in used:
            used[k] += 1
    for k, n in sente_hand.items():
        if k in used:
            used[k] += int(n)

    rem = {}
    for k, total in TOTAL_POOL.items():
        r = total - used.get(k, 0)
        if r > 0:
            rem[k] = r
    return rem

def hand_dict_to_piyo(hand: Dict[str,int]) -> str:
    """Format hand dict like '飛二 金二 ...' (same style as hands_to_piyo)."""
    order = ["R","B","G","S","N","L","P"]
    inv = {
...
def main():
    ...
    def run_example():
        nonlocal start_snapshot, last_piece_token, end_result
    ...
    while True:
        ...
        if s.startswith("end"):
            t = s.split()
            if len(t) == 1:
                print("[end] 終局理由を選んでください: 1) 詰み  2) 投了")
                sel = input("選択: ").strip()
                if sel == "1":
                    end_result = "詰み"
                elif sel == "2":
                    ...
                    print("キャンセルしました（1か2を入力してください）")
                    continue
            else:
                arg = t[1].lower()
                if arg in ("mate", "tsumi", "checkmate"):
                    end_result = "詰み"
                elif arg in ("resign", "toryo", "toryou"):
                    end_result = "投了"
                else:
                    print("end mate / end resign / end を使ってください")
                    continue
            print(f"OK: 末尾行を「{end_result}」に設定しました")
            continue
        ...
        if s.startswith("s "):
            ...
            rem_hand = remaining_gote_hand_from_setup(board0, hands0.get('B', {}))
            out.append(f"後手の持駒：{hand_dict_to_piyo(rem_hand)}")
            ...
            ss = total_sec % 60
            total_str = f"{hh:02d}:{mm:02d}:{ss:02d}"
            out.append(f"{len(pos.moves)+1:4d} {end_result}         ( 0:01/{total_str})")
            text = "\n".join(out) + "\n"
