    ...
    while True:
        ...
        if s.startswith("solve"):
            ...
            if out:
                ...
                stem = Path(out).stem
                written: List[Path] = []
                for i_line, mvlist in enumerate(paths, start=1):
                    fname = f"{stem}_{i_line:03d}.kif"
                    written.append(generate_kif_single_line(board0, hands_b, gote_auto, sente, gote, mvlist, fname))
                print(f"[solve] KIFを {len(written)} 本、OUTPUT に保存しました: {stem}_###.kif")
            else:
