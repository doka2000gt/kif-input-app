...
def main():
    ...
    while True:
        ...
        if s.startswith("batch"):
            t = s.split()
            if len(t) not in (2,3):
                print("形式: batch <dir|file.kif> [ply]\n  例: batch INPUT 9\n      batch some.kif 3")
                continue
            target = t[1]
            ply = None
            if len(t) == 3:
                try:
                    ...
                    ply = None
            # limits use the same defaults as solve
            ...
            try:
                ...
                print(f"[batch] 出力: {len(written)} ファイル -> OUTPUT/")
            except Exception as e:
                print(f"[batch] エラー: {e}")
            continue
        ...
        if s.startswith("solve"):
            ...
            tree = solve_mate_tree(b, ply_left=ply, attacker_turn=attacker_turn, check_only=True,
                                  memo={}, stats=stats, limits=limits)
