...
def format_total_time(total_sec: int) -> str:
    """Format cumulative time like 00:00:SS (ANKIF用途の簡易版)."""
    if total_sec < 0:
        total_sec = 0
    h = total_sec // 3600
    m = (total_sec % 3600) // 60
...
def _dedup_key_from_kif_text(kif_text: str) -> str:
    """Dedup key for final KIF content (hash of cp932 bytes)."""
    b = kif_text.encode("cp932", errors="replace")
    return hashlib.sha1(b).hexdigest()

def _write_kif_unique(outdir: Path, filename: str, kif_text: str,
                      seen: Optional[Dict[str, str]] = None) -> Optional[str]:
    """Write KIF to outdir unless identical content was already written.
    Returns written path (str) or None when skipped as duplicate.
    """
...
def _ensure_output_dir() -> Path:
    outdir = _resolve_kif_path("OUTPUT")
...
def _write_text(outname: str, body_lines: List[str]) -> Optional[str]:
    # keep header up to moves header line, then body_lines
    lines = txt.splitlines()
    keep: List[str] = []
    for ln in lines:
        keep.append(ln)
    ...
    return _write_kif_unique(outdir, outname, out_txt, seen_kif)

        _w = _write_text(f"{base}.kif", main_lines)
        if _w:
            written.append(_w)
        for j, (sp, mvlines) in enumerate(variations, start=2):
            ...
            _w = _write_text(f"{base}_{j:03d}.kif", combined)
            if _w:
                written.append(_w)
        return written
    ...
    # produce move lists from tree
    tree = prune_tree_to_max_leaves(tree, limits.max_solutions if limits else 300)
    lines = enumerate_mate_lines(board0, tree, attacker_turn=pyshogi.BLACK)

    sec_per_move = 3

    # Write main as original (branchless) file, and additional mate lines as _002, _003...
    main_dst = outdir / f"{base}.kif"
    main_dst.write_bytes(raw)
    written.append(str(main_dst))

    for j, mvlist in enumerate(lines[1:], start=2):
        fname = f"{base}_{j:03d}.kif"
        kif_text = build_kif_single_line_text(board0, hands_b, gote_auto, \"\", \"\", mvlist, dt=dt)
        _w = _write_kif_unique(outdir, fname, kif_text, seen_kif)
        if _w:
            written.append(_w)
    print(f"[batch] {path}: 解答筋={len(lines)} / 探索={elapsed:.3f}s nodes={stats['nodes']} -> OUTPUT")
    return written
...
def build_kif_single_line_text(board0, hands_b: Dict[str,int], gote_hands_auto: Dict[str,int],
                               sente_name: str, gote_name: str, moves: List[object],
                               end_word: str = "詰み", dt: Optional[str] = None) -> str:
    """Build a single-line (no-variation) KIF text."""
    if dt is None:
        dt = now_yyyy_mm_dd_hhmmss()

    header: List[str] = []
    header.append("# ---- Kifu for Mac V0.53 夢の詰将棋メーカー by CUI ----")
    header.append(f"終了日時：{dt}")
    ...
    # End line
    end_line = f"{idx:4d} {end_word:<12} ( 0:{sec_per_move:02d}/{format_total_time(total_sec)})"
...
def generate_kif_single_line(board0, hands_b: Dict[str,int], gote_hands_auto: Dict[str,int],
                             sente_name: str, gote_name: str, moves: List[object],
                             outfile: str, dt: Optional[str] = None) -> Path:
    """Write a single-line KIF (no variations)."""
    outdir = _ensure_output_dir()
    kif_text = build_kif_single_line_text(board0, hands_b, gote_hands_auto, sente_name, gote_name, moves, dt=dt)
    outp = _resolve_kif_path(outfile)
    outp.write_bytes(kif_text.encode("cp932", errors="replace"))
    return outp
...
dt = now_yyyy_mm_dd_hhmmss()
seen_kif: Dict[str,str] = {}
# write stem.kif then stem_002...
kif_text = build_kif_single_line_text(board0, hands_b, gote_auto, sente, gote, paths[0], dt=dt)
p0 = _write_kif_unique(outdir, f"{stem}.kif", kif_text, seen_kif)
written: List[str] = []
if p0:
    written.append(p0)
for i_line, mvlist in enumerate(paths[1:], start=2):
    fname = f"{stem}_{i_line:03d}.kif"
    kif_text = build_kif_single_line_text(board0, hands_b, gote_auto, sente, gote, mvlist, dt=dt)
    pw = _write_kif_unique(outdir, fname, kif_text, seen_kif)
    if pw:
        written.append(pw)
                    if len(written) == 1:
