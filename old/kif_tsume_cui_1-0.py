...
def main():
    ...
    while True:
        ...
        if s.startswith("s "):
            ...
            out: List[str] = []
            out.append("# ----  ぴよ将棋 棋譜ファイル  ----")
            out.append("棋戦：")
            out.append("開始日時：" + datetime.datetime.now().strftime("%Y年%m月%d日(%a) %H:%M:%S"))
            out.append("手合割：平手")
            ...
            for i, mv in enumerate(pos.moves, start=1):
                ...
                out.append(tmp.move_to_kif_line(i, tmp.moves[-1]))

            out.append(f"まで{len(pos.moves)}手")
            text = "\n".join(out) + "\n"
